...
if __name__ == '__main__':
    ...
    optional_params = [
        ('--previous-cage', EnvParam(env_name="PREVIOUS_CAGE", cast_type=bool, required=False, default=False).value),
        ('--rpc-timeout', EnvParam(env_name="RPC_TIMEOUT", cast_type=int, required=False).value),
        ('--vat-deployment-block', EnvParam(env_name="FROM_BLOCK", cast_type=int, required=False, default=10310344).value),
    ...
    CageKeeper(keeper_args).main()
    print(f"CageKeeper {keeper_args}")
